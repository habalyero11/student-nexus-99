// Inline CORS headers (no external dependencies)
const corsHeaders = {
...
async function predictStudents(students: StudentData[]): Promise<Array<{
    ...
    const results: any[] = [];
    const studentsToPredict: StudentData[] = [];

    // 1. Check cache for each student
    const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

    // We process students one by one or in batches to check cache
    // For simplicity/performance, let's query all valid predictions for these student IDs
    ...
    for (const student of students) {
        const cached = cachedPredictions?.find(p => p.student_id === student.id);
        if (cached) {
            results.push({
            ...
            // Only predict for students who might be at risk
            if (student.averageGrade < 85 || student.trend === 'declining' || student.attendanceRate < 90) {
                studentsToPredict.push(student);
            }
        }
    }
    ...
    // 2. Call Gemini for missing students
    const studentSummaries = studentsToPredict.map(s =>
    ...
    try {
        ...
        if (jsonMatch) {
            ...
            const newPredictions = (parsed.students || []).map((s: any) => {
                const student = studentsToPredict.find(st => st.name === s.name);
            ...
            results.push(...newPredictions);

            // 3. Save new predictions to cache (fire and forget or safe await)
            if (newPredictions.length > 0) {
                const rowsToInsert = newPredictions.map((p: any) => ({
                ...
                // We wrap this in a separate try-catch so it doesn't block returning results
                try {
                    const { error } = await supabase.from('daily_predictions').insert(rowsToInsert);
